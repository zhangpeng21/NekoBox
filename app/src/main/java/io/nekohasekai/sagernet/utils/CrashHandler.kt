...
object CrashHandler : Thread.UncaughtExceptionHandler {
    ...
    fun buildReportHeader(): String {
        ...
        try {
            report += "Settings: \n"
            // Fix: exclude keys that may contain sensitive proxy credentials from crash reports
            val sensitiveKeySubstrings = listOf(
                "password", "Password", "privateKey", "private_key", "secret", "Secret",
                "certificates", "Certificates", "preshared", "PreShared"
            )
            for (pair in PublicDatabase.kvPairDao.all()) {
                val isSensitive = sensitiveKeySubstrings.any { pair.key.contains(it) }
