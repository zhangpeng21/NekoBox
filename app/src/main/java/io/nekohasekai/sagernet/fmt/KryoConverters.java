...
public class KryoConverters {
    ...
    // Deep-Opt: Pool ByteArrayOutputStream and ByteBufferOutput per-thread.
    // serialize() is called on EVERY DB write, equals(), hashCode(), clone(), and
    // link export. Previously each call allocated two heap objects (BAOS + BBOutput)
    // which immediately became garbage.  With thread-local pooling:
    //  - zero allocation on subsequent calls from the same thread
    //  - BAOS is reset (position=0) not reallocated; buffer grows once then stays
    //  - ByteBufferOutput is reset and reused
    private static final ThreadLocal<ByteArrayOutputStream> tlBaos =
        ThreadLocal.withInitial(() -> new ByteArrayOutputStream(256));

    private static final ThreadLocal<ByteBufferOutput> tlOutput =
        ThreadLocal.withInitial(() -> {
            ByteBufferOutput out = KryosKt.byteBuffer(tlBaos.get());
            return out;
        });
    ...
    public static byte[] serialize(Serializable bean) {
        ...
        // Re-use pooled BAOS+Output.  We must reset BAOS BEFORE resetting the Output
        // because ByteBufferOutput holds an internal reference to the stream position.
