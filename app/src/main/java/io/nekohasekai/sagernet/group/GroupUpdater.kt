...
abstract class GroupUpdater {
    ...
    protected suspend fun forceResolve(
        ...
        for (profile in profiles) {
            ...
            // Opt: use the shared DNS dispatcher instead of creating a new thread pool per
            // update call. Creating newFixedThreadPoolContext() allocates 5 threads and an
            // executor on every subscription refresh, and tears them all down afterwards.
            // Deep-Opt: use AppScope instead of GlobalScope to avoid coroutine leaks.
            lookupJobs.add(AppScope.launch(sharedDnsDispatcher) {
                try {
                    val results = if (
    ...
    companion object {
        ...
        // Opt: shared, reusable dispatcher for parallel DNS lookups.
        // Previously a new thread pool was created and destroyed on every subscription refresh.
        private val sharedDnsDispatcher = newFixedThreadPoolContext(5, "DNS-Lookup-Shared")
