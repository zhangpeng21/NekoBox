...
object RawUpdater : GroupUpdater() {
    ...
    override suspend fun doUpdate(
        ...
        if (link.startsWith("content://")) {
            // Fix: wrap in use{} so the InputStream is always closed, preventing resource leak
        ...
        // Deep-Opt: counter map for O(1) duplicate-name resolution (see loop below)
        val collisionCounters = HashMap<String, Int>()
        val proxiesMap = LinkedHashMap<String, AbstractBean>()
        for (proxy in proxies) {
            val baseName = proxy.displayName()
            if (!proxiesMap.containsKey(baseName)) {
                proxiesMap[baseName] = proxy
            } else {
                // Deep-Opt: previously used a while-loop that called String.replace()
                // on each iteration to strip the previous "(N)" suffix, making it O(n²)
                // when many proxies share a base name.  Instead we track the counter in
                // a separate map and build the suffixed name in O(1) per collision.
                val counter = collisionCounters.getOrDefault(baseName, 1) + 1
                collisionCounters[baseName] = counter
                val uniqueName = "$baseName ($counter)"
                proxy.name = uniqueName
                proxiesMap[uniqueName] = proxy
            }
        }
    ...
    suspend fun parseRaw(text: String, fileName: String = ""): List<AbstractBean>? {
        ...
        if (text.contains("proxies:")) {
            ...
            try {
                ...
                // Fix: use SafeConstructor to prevent SnakeYAML deserialization of arbitrary Java
                // types, which can lead to unexpected behavior or denial-of-service with crafted input.
