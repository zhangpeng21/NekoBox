...
abstract class BoxInstance(
    ...
    companion object {
        // Fix: use an atomic counter instead of timestamps for temp file names.
        // nextFileId() has millisecond granularity and can produce
        // identical values when multiple files are created in quick succession,
        // leading to file name collisions.
        private val fileIdCounter = AtomicLong(0)
        fun nextFileId(): Long = fileIdCounter.incrementAndGet()
    }
    ...
    // Fix: use CopyOnWriteArrayList so concurrent coroutines can safely add cache files
    private val cacheFiles = java.util.concurrent.CopyOnWriteArrayList<File>()
                ...
                when (val bean = profile.requireBean()) {
                    ...
                    is HysteriaBean -> {
                        ...
                        pluginConfigs[port] = profile.type to bean.buildHysteria1Config(port) {
                            File(
                                app.cacheDir, "hysteria_" + nextFileId() + ".ca"
                            ).apply {
                                parentFile?.mkdirs()
                                cacheFiles.add(this)
                            }
            ...
            chain.entries.forEachIndexed { index, (port, profile) ->
                ...
                when {
                    ...
                    bean is TrojanGoBean -> {
                        val configFile = File(
                            cacheDir, "trojan_go_" + nextFileId() + ".json"
                        )
                    ...
                    bean is MieruBean -> {
                        val configFile = File(
                            cacheDir, "mieru_" + nextFileId() + ".json"
                        )
                    ...
                    bean is NaiveBean -> {
                        val configFile = File(
                            cacheDir, "naive_" + nextFileId() + ".json"
                        )
                        ...
                        if (bean.certificates.isNotBlank()) {
                            val certFile = File(
                                cacheDir, "naive_" + nextFileId() + ".crt"
                            )
                    ...
                    bean is HysteriaBean -> {
                        val configFile = File(
                            cacheDir, "hysteria_" + nextFileId() + ".json"
                        )
    ...
    override fun close() {
        ...
        if (::processes.isInitialized) processes.close(AppScope)
