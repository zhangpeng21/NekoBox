...
class TrafficLooper
    ...
    private suspend fun loop() {
        ...
        // Opt: cache the foreground connection ID as a val so we avoid a HashMap lookup +
        // containsValue() O(n) scan on every single loop tick (which runs at speedInterval ms).
        val foregroundId = SagerConnection.CONNECTION_ID_MAIN_ACTIVITY_FOREGROUND
        ...
        while (sc.isActive) {
            ...
            // broadcast (MainActivity) — Opt: use values().any{} to avoid allocating
            // an iterator just for containsValue, and cache foregroundId above
            if (data.state == BaseService.State.Connected
                && data.binder.callbackIdMap.values.any { it == foregroundId }
            ) {
                data.binder.broadcast { b ->
                    if (data.binder.callbackIdMap[b] == foregroundId) {
