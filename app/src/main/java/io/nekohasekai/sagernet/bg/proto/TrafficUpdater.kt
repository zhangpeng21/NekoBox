...
class TrafficUpdater(
    ...
    companion object {
        // Opt: intern direction constant strings so the JNI layer receives the same
        // String object reference on every call rather than a freshly allocated one.
        private val DIR_UPLINK = "uplink".intern()
        private val DIR_DOWNLINK = "downlink".intern()
    }
    ...
    private fun updateOne(item: TrafficLooperData): TrafficLooperData {
        ...
        // query — Opt: use interned constants instead of new string literals each call
        val tx = box.queryStats(item.tag, DIR_UPLINK)
        val rx = box.queryStats(item.tag, DIR_DOWNLINK)
