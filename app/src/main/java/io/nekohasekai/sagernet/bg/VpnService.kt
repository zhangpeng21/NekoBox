...
//        val tunOptions = JSONObject(tunOptionsJson)
        ...
        // Fix: removed premature setMetered(metered) here — metered field is not yet set from
        // DataStore at this point (still false). The correct call below reads DataStore first.
        ...
        if (proxyApps || needBypassRootUid) {
            ...
            val allApps by lazy {
                // Opt: reuse PackageCache which is already populated during app init,
                // avoiding a redundant getInstalledPackages() IPC call every VPN start.
                PackageCache.installedPackages.keys.filter { it != packageName }
            }
            if (proxyApps) {
