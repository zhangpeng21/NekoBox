...
class ServiceNotification(
    ...
    // Deep-Opt: Track last-posted speed text to skip IPC notify() calls when
    // the displayed content has not changed between ticks.  At 500 ms intervals
    // with stable throughput the text is identical most of the time; skipping
    // notify() avoids a Binder IPC + StatusBar deserialization on every tick.
    ...
    suspend fun postNotificationSpeedUpdate(stats: SpeedDisplayData) {
        val newSpeedText: String
        val newSubText: String

        if (showDirectSpeed) {
            newSpeedText = (service as Context).getString(
        ...
        newSubText = service.getString(
            R.string.traffic,
            Formatter.formatFileSize(service, stats.txTotal),
            Formatter.formatFileSize(service, stats.rxTotal)
        )

        // Skip IPC call when nothing changed — happens often at stable network speed
        if (newSpeedText == lastSpeedText && newSubText == lastSubText) return
        lastSpeedText = newSpeedText
        lastSubText = newSubText

        useBuilder {
            if (showDirectSpeed) {
                it.setStyle(NotificationCompat.BigTextStyle().bigText(newSpeedText))
                it.setContentText(newSpeedText)
            } else {
                it.setContentText(newSpeedText)
            }
            it.setSubText(newSubText)
        }
        update()
    }
