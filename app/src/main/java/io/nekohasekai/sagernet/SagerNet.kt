...
class SagerNet : Application(),
    ...
    override fun onTrimMemory(level: Int) {
        ...
        Libcore.forceGc()
        // Deep-Opt: under memory pressure, flush caches that grow unboundedly.
        // PackageCache.labelMap holds one String per installed app (~100–500 entries);
        // it is rebuilt lazily so clearing it here is safe and free up a few KB.
        if (level >= TRIM_MEMORY_MODERATE) {
            PackageCache.clearLabelCache()
        }
    }
