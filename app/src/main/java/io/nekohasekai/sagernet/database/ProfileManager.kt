...
object ProfileManager {
    ...
    // Opt: use CopyOnWriteArrayList so that the suspend iterator() functions can
    // iterate without holding a lock (the hot path — called on every traffic update
    // tick), while add/remove operations (infrequent) pay the copy cost instead.
    private val listeners = CopyOnWriteArrayList<Listener>()
    private val ruleListeners = CopyOnWriteArrayList<RuleListener>()

    suspend fun iterator(what: suspend Listener.() -> Unit) {
        // No synchronized copy needed — CopyOnWriteArrayList's iterator is snapshot-safe
        listeners.forEach { listener -> what(listener) }
    }

    suspend fun ruleIterator(what: suspend RuleListener.() -> Unit) {
        ruleListeners.forEach { listener -> what(listener) }
    }
