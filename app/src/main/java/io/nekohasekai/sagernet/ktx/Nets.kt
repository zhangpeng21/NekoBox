...
// Note: mkPort() has an inherent TOCTOU (time-of-check/time-of-use) race:
// the port is freed before it can be used by the caller, so another process
// could claim it in the interval. The only complete fix is to keep the socket
// open and pass the fd, but that requires API changes. We add a simple retry
// loop here so at least transient failures don't crash the app.
fun mkPort(): Int {
    repeat(5) { attempt ->
        try {
            val socket = Socket()
            ...
            if (attempt == 4) throw RuntimeException("mkPort: failed to allocate a local port after 5 attempts")
        }
    }
    throw RuntimeException("mkPort: failed to allocate a local port")
}
